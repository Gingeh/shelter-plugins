...
export function onLoad() {
  ...
  unObserveCallbacks.push(
    observeDom(
      `[id*=message-username-]:not([data-${hopefully_unique_id}])`,
      (elem) => {
        ...
        if (
          store.respectRoles &&
          !LayerStore.getLayers().includes("USER_SETTINGS")
        ) {
          const { type, guild_id } = ChannelStore.getChannel(
            message.channel_id
          );
          if (type === 0) {
  ...
  unObserveCallbacks.push(
    observeDom(
      `[class*=nameContainer__]:not([data-${hopefully_unique_id}])`,
      (elem) => {
        elem.setAttribute(`data-${hopefully_unique_id}`, "true");

        const user = reactFiberWalker(getFiber(elem), "user", true)
          ?.pendingProps?.user;
        if (!user) return;
  ...
  unObserveCallbacks.push(
    observeDom(
      `[class*=repliedMessage_] [class*=username_]:not([data-${hopefully_unique_id}])`,
      (elem) => {
        elem.setAttribute(`data-${hopefully_unique_id}`, "true");
